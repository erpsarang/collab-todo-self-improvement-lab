...
test('publisher reconciles a duplicate that becomes visible on a bounded follow-up lookup', async () => {
  ...
  assert.equal(listCalls, 4, 'a creator uses all three bounded reconciliation lookups');
...
test('concurrent publishers discover delayed duplicates throughout the bounded reconciliation window', async () => {
  const script = workflowScript(workflow(), 'Publish candidate record');
  const publication = candidatePublication({ title: 'Reconcile concurrent delayed duplicates' });
  const directory = mkdtempSync(join(tmpdir(), 'candidate-concurrent-visibility-'));
  mkdirSync(join(directory, 'candidate-artifact'));
  writeFileSync(join(directory, 'candidate-artifact/self-improvement-publication.json'), JSON.stringify(publication));
  const issues = [];
  let createdCount = 0;
  ...
  const githubFor = (publisher) => {
    ...
    const api = {
      ...
      listForRepo: async () => {
        ...
        if (listCalls === 2) return { data: [ownIssue] };
        return { data: issues.filter(({ labels }) => labels.includes('SI-후보')) };
      },
      create: async (input) => {
        ownIssue = { ...input, number: publisher === 'first' ? 21 : 22, comments: [] };
        issues.push(ownIssue);
        createdCount += 1;
        if (createdCount === 2) releaseCreates();
        await bothCreated;
        return { data: ownIssue };
      },
      listComments: async ({ issue_number }) => ({ data: issues.find(({ number }) => number === issue_number).comments }),
      createComment: async ({ issue_number, body }) => { issues.find(({ number }) => number === issue_number).comments.push({ body }); },
      removeLabel: async ({ issue_number, name }) => {
    ...
    return {
      github: { paginate: async (method, input) => (await method(input)).data, rest: { issues: api } },
      getListCalls: () => listCalls,
    };
  };
  const first = githubFor('first');
  const second = githubFor('second');
  const execute = (github, runId) => new Function('require', 'github', 'context', 'core', 'process',
    `return (async () => {\n${script}\n})()`)(require, github, { repo: {} }, { info() {} }, { env: { SOURCE_RUN_ID: String(runId) } });
  const previousCwd = process.cwd();
  process.chdir(directory);
  try {
    await Promise.all([execute(first.github, 1101), execute(second.github, 1102)]);
  } finally {
    process.chdir(previousCwd);
  }

  assert.equal(first.getListCalls(), 4, 'the first publisher uses the complete bounded lookup window');
  assert.equal(second.getListCalls(), 4, 'the second publisher uses the complete bounded lookup window');
  assert.deepEqual(issues.find(({ number }) => number === 21).labels, ['SI-후보', 'SI-승인대기']);
  assert.deepEqual(issues.find(({ number }) => number === 22).labels, ['SI-승인대기']);
  assert.equal(issues.find(({ number }) => number === 22).state, 'closed');
});
